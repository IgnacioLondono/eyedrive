...
const { mountTwoFactorRoutes, startPending2faLogin } = require("./twoFactor");
  ...
  return async function loadSession(req, res, next) {
    ...
    try {
      const { rows } = await pool.query(
        `SELECT u.id, u.email, u.display_name, u.email_verified, u.created_at, u.totp_enabled
         FROM sessions s
      ...
      if (rows.length) {
        req.user = {
          ...
          twoFactorEnabled: Boolean(rows[0].totp_enabled),
        };
...
function mountAuthRoutes(app, pool, { sendVerificationCode }) {
  ...
  app.post("/api/auth/password/confirm", async (req, res) => {
    ...
    try {
      ...
      const { rows } = await pool.query(
        `SELECT id, email, display_name, email_verified FROM users WHERE email = $1 AND email_verified = TRUE`,
        [email]
      );
      if (!rows.length) {
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    try {
      const { rows } = await pool.query(
        `SELECT id, email, display_name, email_verified, password_hash, totp_enabled FROM users WHERE email = $1`,
        [email]
      );
      if (!rows.length) {
      ...
      if (user.totp_enabled) {
        const pendingToken = await startPending2faLogin(pool, user.id);
        return res.json({
          requires2fa: true,
          pendingToken,
