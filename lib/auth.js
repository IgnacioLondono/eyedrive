...
function mountAuthRoutes(app, pool, { sendVerificationCode }) {
  ...
  app.post("/api/auth/register/request", async (req, res) => {
    ...
    try {
      ...
      res.json({ ok: true, message: "Te hemos enviado un código de confirmación" });
    } catch (e) {
      console.error(e);
      if (e.code === "EAUTH") {
        return res.status(503).json({
          error: "El servidor no puede enviar correos. Revisa SMTP_USER y SMTP_PASS (contraseña de aplicación de Gmail).",
        });
      }
  ...
  app.post("/api/auth/login/request", async (req, res) => {
    ...
    try {
      ...
      if (!rows.length) {
        return res.status(404).json({ error: "No hay ninguna cuenta con ese correo. Regístrate primero." });
      }
      ...
      res.json({ ok: true, message: "Te hemos enviado un código de acceso" });
    } catch (e) {
      console.error(e);
      if (e.code === "EAUTH") {
        return res.status(503).json({
