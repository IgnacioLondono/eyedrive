...
const DEVICE_TRUST_DAYS = Number(process.env.DEVICE_TRUST_DAYS || 90);
const APP_URL = (process.env.APP_URL || "http://localhost:9990").replace(/\/$/, "");
const DEVICE_PEPPER = process.env.DEVICE_PEPPER || APP_URL || "eyedrive-device-v1";
const BCRYPT_ROUNDS = 12;
const DEVICE_ID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const UUID_RE = DEVICE_ID_RE;
...
function resolveDeviceId(req) {
  return normalizeDeviceId(req.body?.deviceId || req.get("x-device-id"));
}

function hashDeviceId(userId, deviceId) {
  return crypto
    .createHash("sha256")
    .update(`${userId}:${deviceId}:${DEVICE_PEPPER}`)
    .digest("hex");
}

function parseDeviceLabel(req) {
...
function deviceTrustExpiryDate() {
  const d = new Date();
  d.setDate(d.getDate() + DEVICE_TRUST_DAYS);
  return d;
}
...
function clearSessionCookie(req, res) {
  res.clearCookie(SESSION_COOKIE, { path: "/", secure: isSecureRequest(req) });
}

function resolveSessionToken(req) {
  const header = String(req.get("x-session-token") || "").trim();
  if (header) return header;
  const query = String(req.query?.st || "").trim();
  ...
  return async function loadSession(req, res, next) {
    ...
    try {
      const { rows } = await pool.query(
        `SELECT u.id, u.email, u.display_name, u.email_verified, u.login_code_enabled, u.created_at
         FROM sessions s
      ...
      if (rows.length) {
        req.user = {
          ...
          loginCodeEnabled: rows[0].login_code_enabled,
...
async function provisionUserDrive(pool, userId) {
...
async function isTrustedDevice(pool, userId, deviceId) {
  const normalized = normalizeDeviceId(deviceId);
  if (!normalized) return false;
  const deviceHash = hashDeviceId(userId, normalized);
  const { rows } = await pool.query(
...
async function trustDevice(pool, userId, deviceId, req) {
  const normalized = normalizeDeviceId(deviceId);
  if (!normalized) return;
  const deviceHash = hashDeviceId(userId, normalized);
  const expiresAt = deviceTrustExpiryDate();
  await pool.query(
    `INSERT INTO trusted_devices (user_id, device_hash, device_label, user_agent, expires_at)
     VALUES ($1, $2, $3, $4, $5)
...
async function clearTrustedDevices(pool, userId) {
  await pool.query(`DELETE FROM trusted_devices WHERE user_id = $1`, [userId]);
}

async function finishLogin(req, res, pool, user) {
  await provisionUserDrive(pool, user.id);
...
function emailSendError(res, e) {
  ...
  return res.status(503).json({
    error: "El servidor no puede enviar correos. Revisa la configuración de Resend.",
  });
}
...
function mountAuthRoutes(app, pool, { sendVerificationCode }) {
  ...
  app.get("/api/auth/me", (req, res) => {
    ...
    res.json({
      ...
      loginCodeEnabled: Boolean(req.user.loginCodeEnabled),
    });
  });
    ...
    try {
      ...
      if (!rows[0].email_verified) {
        return res.status(403).json({
          error: "Esta cuenta aún no está confirmada. Completa el registro primero.",
        });
      }
      const code = await storeEmailCode(pool, email, "reset");
  ...
  app.post("/api/auth/password/confirm", async (req, res) => {
    ...
    try {
      const ok = await verifyEmailCode(pool, email, code, "reset");
      if (!ok) return res.status(400).json({ error: "Código incorrecto o caducado" });

      const { rows } = await pool.query(
        `SELECT id, email, display_name, email_verified, login_code_enabled
      ...
      await clearTrustedDevices(pool, user.id);
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    try {
      const { rows } = await pool.query(
        `SELECT id, email, display_name, email_verified, password_hash, login_code_enabled
      ...
      if (!user.login_code_enabled) {
        return finishLogin(req, res, pool, user);
      }

      if (deviceId && (await isTrustedDevice(pool, user.id, deviceId))) {
        return finishLogin(req, res, pool, user);
      }

      const code = await storeEmailCode(pool, email, "login");
      await sendVerificationCode(email, code, "login");
      res.json({
        ok: true,
        needsCode: true,
        email,
        message: "Te hemos enviado un código para completar el inicio de sesión.",
      });
    } catch (e) {
      return emailSendError(res, e);
    }
  ...
  app.post("/api/auth/login/confirm", async (req, res) => {
    ...
    try {
      const ok = await verifyEmailCode(pool, email, code, "login");
      if (!ok) return res.status(400).json({ error: "Código incorrecto o caducado" });
      ...
      const user = rows[0];
      if (deviceId) {
        await trustDevice(pool, user.id, deviceId, req);
      }
      return finishLogin(req, res, pool, user);
    } catch (e) {
      console.error(e);
  ...
  app.post("/api/auth/login/resend", async (req, res) => {
    ...
    try {
      ...
      const { rows: pendingLogin } = await pool.query(
        `SELECT id FROM email_codes
      ...
      if (!pendingLogin.length) {
        return res.status(400).json({
          error: "Vuelve a iniciar sesión con tu correo y contraseña para recibir un código nuevo.",
        });
      }
  ...
  app.post("/api/auth/logout", async (req, res) => {
    try {
      await destroySession(pool, req.sessionToken || resolveSessionToken(req));
    } catch (e) {
      console.error(e);
    }
  ...
  app.patch("/api/auth/account/password", requireAuth, async (req, res) => {
    ...
    try {
      ...
      await clearTrustedDevices(pool, req.user.id);
      res.json({ ok: true });
    } catch (e) {
      console.error(e);
  ...
  app.patch("/api/auth/account/security", requireAuth, async (req, res) => {
    if (typeof req.body?.loginCodeEnabled !== "boolean") {
      return res.status(400).json({ error: "Indica si quieres activar el código de acceso" });
    }
    const enabled = req.body.loginCodeEnabled;
    const deviceId = resolveDeviceId(req);
    try {
      await pool.query(
        `UPDATE users SET login_code_enabled = $2, updated_at = NOW() WHERE id = $1`,
        [req.user.id, enabled]
      );
      if (enabled) {
        if (deviceId) {
          await trustDevice(pool, req.user.id, deviceId, req);
        }
      } else {
        await clearTrustedDevices(pool, req.user.id);
      }
      res.json({ ok: true, loginCodeEnabled: enabled });
    } catch (e) {
      console.error(e);
      res.status(500).json({ error: "No se pudo actualizar la seguridad" });
    }
  });

  app.get("/api/auth/account/devices", requireAuth, async (req, res) => {
    const deviceId = normalizeDeviceId(req.query?.deviceId || req.get("x-device-id"));
    const currentHash = deviceId ? hashDeviceId(req.user.id, deviceId) : "";
    try {
      const { rows } = await pool.query(
        `SELECT id, device_label, user_agent, last_used_at, created_at,
                (device_hash = $2) AS is_current
      ...
      res.json({
        devices: rows.map((row) => ({
          id: row.id,
          label: row.device_label || "Navegador web",
  ...
  app.delete("/api/auth/account/devices/:id", requireAuth, async (req, res) => {
    ...
    try {
      ...
      console.error(e);
      res.status(500).json({ error: "No se pudo quitar el dispositivo" });
    }
  });

  app.delete("/api/auth/account/devices", requireAuth, async (req, res) => {
    try {
      await clearTrustedDevices(pool, req.user.id);
      res.json({ ok: true });
    } catch (e) {
      console.error(e);
      res.status(500).json({ error: "No se pudieron quitar los dispositivos" });
    }
  });
